...
class OutgoingContactChatCell: OutgoingChatCell {
    ...
    func fill(for message: OutgoingChatMessage, onClickAction: (()->())?) {
        ...
        if let contacts = message.attachment?.contacts, let contact = contacts.first {
            ...
            contactPhonesAndEmailsLabel.text = OutgoingContactChatCell.getPhonesAndEmailsStr(for: contact)
        } else {
            contactImage.image = UIImage(named: "avatar_placeholder")
    ...
    static func getHeight(_ message: OutgoingChatMessage) -> CGFloat {
        let totalTopBottomConstarints: CGFloat = 48
        let totalLeftRightConstraints: CGFloat = 183
        ...
        if let contacts = message.attachment?.contacts, let contact = contacts.first {
            ...
            return totalTopBottomConstarints
